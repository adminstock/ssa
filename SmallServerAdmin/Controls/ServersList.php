  ...
  <tbody>
    <tr ng-repeat="server in Servers" ng-class="{'success': (server.Address == Config.ServerAddress && server.Name == Config.ServerName) && !CurrentServerConnectionError && !ConnectionTesting, 'danger': (server.Address == Config.ServerAddress && server.Name == Config.ServerName) && CurrentServerConnectionError, 'warning': (server.Address == Config.ServerAddress && server.Name == Config.ServerName) && ConnectionTesting, 'text-muted': server.Disabled}">
      ...
      <td class="col-xs-2 col-sm-2 col-md-2 col-lg-2">
        <button type="button" class="btn col-xs-12 col-sm-12 col-md-12 col-lg-12" ng-class="server.Disabled ? 'btn-gray' : 'btn-primary'" ng-show="server.Address != Config.ServerAddress || server.Name != Config.ServerName" ng-click="ConnectToServer(server)" ng-disabled="server.Disabled">
          <span ng-show="server.Disabled">${Disabled}</span>
          <span ng-hide="server.Disabled">${Connect}</span>
        </button>
        <button type="button" class="btn btn-success col-xs-12 col-sm-12 col-md-12 col-lg-12" ng-show="(server.Address == Config.ServerAddress && server.Name == Config.ServerName) && !CurrentServerConnectionError && !ConnectionTesting" disabled="disabled">${Current}</button>
        <button type="button" class="btn btn-danger col-xs-12 col-sm-12 col-md-12 col-lg-12" ng-show="(server.Address == Config.ServerAddress && server.Name == Config.ServerName) && CurrentServerConnectionError" disabled="disabled">${Current}</button>
        <button type="button" class="btn btn-warning col-xs-12 col-sm-12 col-md-12 col-lg-12" ng-show="(server.Address == Config.ServerAddress && server.Name == Config.ServerName) && ConnectionTesting" disabled="disabled">
